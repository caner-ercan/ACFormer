...
class CellDetDataset_Lizard_Best(CustomDataset):
    # CLASSES = ("Neutrophil", "Epithelial", "Lymphocyte", "Plasma",  "Connective")
    ...
    def evaluate(self,
        ...
        for idx, result in enumerate(results):
            ...
            col_offset = img_info[0]["height"]
            raw_offset = img_info[0]["width"]

            if idx % 4 == 0:
                # img_path = os.path.join("/mntnfs/med_data2/huangjunjia/dataset/HoverNet_BRCA/Test/Labels/",
                #                         img_name[:-7] + ".mat")
            ...
            for i in range(len(self.cat_ids)):
                ...
                if idx % 4 == 0:
                    classes[:, 0] += 0 * raw_offset
                    classes[:, 1] += 0 * col_offset
                elif idx % 4 == 1:
                    classes[:, 0] += 1 * raw_offset
                    classes[:, 1] += 0 * col_offset
                elif idx % 4 == 2:
                    classes[:, 0] += 0 * raw_offset
                    classes[:, 1] += 1 * col_offset
                elif idx % 4 == 3:
                    classes[:, 0] += 1 * raw_offset
                    classes[:, 1] += 1 * col_offset
            ...
            if idx % 4 == 3:
                ...
                true_idx_offset = (
                    true_idx_offset + true_inst_type_all[-1].shape[0] if idx != 3 else 0
                )
                pred_idx_offset = (
                    pred_idx_offset + pred_inst_type_all[-1].shape[0] if idx != 3 else 0
                )
        ...
        paired_all = np.concatenate(paired_all, axis=0) if len(paired_all) else np.empty((0, 1))
        unpaired_true_all = np.concatenate(unpaired_true_all, axis=0)
        unpaired_pred_all = np.concatenate(unpaired_pred_all, axis=0)
        true_inst_type_all = np.concatenate(true_inst_type_all, axis=0)
        pred_inst_type_all = np.concatenate(pred_inst_type_all, axis=0)

        paired_true_type = true_inst_type_all[paired_all[:, 0]] if len(paired_all) else np.empty((0, 1))
        paired_pred_type = pred_inst_type_all[paired_all[:, 1]] if len(paired_all) else np.empty((0, 1))
