...
def trans_to_coco(patch_save_img_path, qp_input_csv_path, qp_output_json_path):
    ...
    for imgid, image in enumerate(imgs_files):
        ...
        classes_ann = pd.read_csv(os.path.join(qp_input_csv_path, os.path.splitext(image)[0] +'.csv'))
        # for annid, (centroid, type) in enumerate(zip(classes_ann["inst_centroid"], classes_ann["inst_type"])):
        for annid, (x, y, cell_type) in classes_ann.iterrows():
            bbox = [x - 2.0, y - 2.0, 4.0, 4.0]
            area = 16
            # type = int(type)
            cellType = cell_type_dict[cell_type]
    ...
    with open(qp_output_json_path, "w") as write_f:
...
def split_rois_csv(qp_input_img_path,qp_input_csv_path,output_folder):
    ...
    img_list = list(set(img_list))  # Get a list of unique names

    #split images
    random.shuffle(img_list)  
    split_index = int(0.8 * len(img_list))
    train_img_list = img_list[:split_index]  
    val_img_list = img_list[split_index:]

    #move files
    for dataset_type in ["training", "validation"]:
        os.makedirs(os.path.join(output_folder,dataset_type,"roi"), exist_ok =True)
        os.makedirs(os.path.join(output_folder,dataset_type,"csv"), exist_ok =True)
    
    for filecorename in train_img_list:
        for filename in os.listdir(qp_input_img_path):
            if filename.startswith(filecorename):
                shutil.copy(os.path.join(qp_input_img_path, filename), os.path.join(output_folder,"training","roi"))
                shutil.copy(os.path.join(qp_input_csv_path, os.path.splitext(filename)[0] + '.csv'), os.path.join(output_folder,"training","csv"))
    for filecorename in val_img_list:
        for filename in os.listdir(qp_input_img_path):
            if filename.startswith(filecorename):
                shutil.copy(os.path.join(qp_input_img_path, filename), os.path.join(output_folder,"validation","roi"))
                shutil.copy(os.path.join(qp_input_csv_path, os.path.splitext(filename)[0] + '.csv'), os.path.join(output_folder,"validation","csv"))
...
if __name__ == '__main__':
    ...
    qp_input_img_path = "/rsrch5/home/trans_mol_path/cercan/data/BE/cell_detect/acformer/finetune/qpproj/roi" #coco_save_img_path
    qp_input_csv_path = "/rsrch5/home/trans_mol_path/cercan/data/BE/cell_detect/acformer/finetune/qpproj/corrected_point" #coco_save_gt_path
    ...
    patch_save_img_path = qp_input_img_path #coco_save_img_path
    patch_save_gt_path = qp_input_csv_path #coco_save_gt_path

    # hover_save_gt_path = "/data2/huangjunjia/coco/CoNSeP/{}/gt_mat".format(mode[1]) #to split big patches
    ...
    # classes_max_indx = 3
    output_folder="/rsrch5/home/trans_mol_path/cercan/data/BE/cell_detect/acformer/finetune/qpproj/finetuning"
